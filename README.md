...
The `simDataFileList.csv` file (a list of all leftover simulation environments booted from the HPC queue/contained in the `simData` directory) may be generated by running the bash script found [**here**](https://github.com/nathdep/SMEP24/blob/main/genSimDataList.sh). 
...
selectedFile <- read.csv("simDataFileList.csv")[args[2],1]
